/* ══════════════════════════════════════
        ...
        async function loadData() {
            ...
            DB.spots = spotsData.map(mapSpotFromDb);
            DB.owners = ownersData.map(mapOwnerFromDb);
            buildActivityHistory();
        }

        function buildActivityHistory() {
            DB.history = DB.spots.slice(0, 6).map(s => ({
                action: `Actualizado ${s.number}`,
                user: `${ADMIN.nombre} ${ADMIN.apellidos}`,
                time: s.createdAt || '—'
            }));
        }
        ...
        async function saveOwner() {
            ...
            const data = {
                fullName,
                phone: document.getElementById('ow-tel').value.trim() || '—',
                email: document.getElementById('ow-email').value.trim() || '—',
                portal: document.getElementById('ow-portal').value,
                vivienda: document.getElementById('ow-vivienda').value.trim() || '—',
                obs: document.getElementById('ow-obs').value.trim()
            };
            try {
                if (ST.editOwner) {
                    const saved = await updateOwner(Number(ST.editOwner), mapOwnerToDb(data));
                    const idx = DB.owners.findIndex(o => o.id === ST.editOwner);
                    if (idx !== -1) DB.owners[idx] = mapOwnerFromDb(saved);
                    showToast('success', 'Actualizado', 'Propietario modificado.');
                } else {
                    const saved = await addOwner(mapOwnerToDb(data));
                    DB.owners.push(mapOwnerFromDb(saved));
        ...
        function confirmDeleteOwner(id) {
            ...
            if (!o) return;
            const spots = DB.spots.filter(s => s.ownerName === o.fullName).length;
            confirmAction(
                'Eliminar propietario',
                `¿Eliminar a <strong>${o.fullName}</strong>?${spots > 0 ? `<br><br><strong style="color:var(--accent-orange)">⚠️ Tiene ${spots} plaza${spots > 1 ? 's' : ''} asignada${spots > 1 ? 's' : ''}</strong>. Quedarán sin titular.` : ''}`,
                async () => {
                    await deleteOwner(Number(id));
                    DB.owners = DB.owners.filter(x => x.id !== id);
                    ST.own.selected.delete(id);
                    updateBadges();
                    renderOwnersGrid();
                    renderKPIs();
                    showToast('success', 'Eliminado', `"${o.fullName}" eliminado.`);
                }
            );
        }
        ...
        function bulkDelete(type) {
            ...
            confirmAction(`Eliminar ${n} ${label}`, `¿Eliminar ${n === 1 ? 'el' : 'los'} ${n} ${label} seleccionado${n > 1 ? 's' : ''}? Esta acción no se puede deshacer.`, async () => {
                const ids = [...sel.selected].map(Number);
                if (type === 'owners') {
                    await deleteOwners(ids);
                    DB.owners = DB.owners.filter(o => !sel.selected.has(o.id));
                } else {
                    await deleteSpots(ids);
                    DB.spots = DB.spots.filter(s => !sel.selected.has(s.id));
                }
                sel.selected.clear();
                updateBadges();
                renderKPIs();
        ...
        /* ══════════════════════════════════════
           SINGLE DELETE SPOT
        ══════════════════════════════════════ */
        function confirmDeleteSingle(id, type) {
            const s = DB.spots.find(x => x.id === id);
            if (!s) return;
            const label = type === 'vehicles' ? 'vehículo' : 'motocicleta';
            confirmAction(`Eliminar ${label}`, `¿Eliminar la plaza <strong>${s.number}</strong> — ${s.ownerName} — <code>${s.plate}</code>?`, async () => {
                await deleteSpot(Number(id));
                DB.spots = DB.spots.filter(x => x.id !== id);
                if (type === 'vehicles') { ST.veh.selected.delete(id); renderVehiclesTable(); }
                else { ST.mot.selected.delete(id); renderMotosTable(); }
                closeDetailPanel();
                updateBadges();
                renderKPIs();
        ...
        async function saveRecord() {
            ...
            try {
                if (ST.editId) {
                    const saved = await updateSpot(Number(ST.editId), mapSpotToDb(data));
                    const idx = DB.spots.findIndex(s => s.id === ST.editId);
                    if (idx !== -1) DB.spots[idx] = mapSpotFromDb(saved);
                    showToast('success', 'Actualizado', 'Registro modificado.');
                } else {
                    const saved = await addSpot(mapSpotToDb(data));
                    DB.spots.push(mapSpotFromDb(saved));
        ...
        async function executeConfirm() {
            if (ST.confirmCb) {
                try {
                    ...
                    showToast('error', 'Error', 'No se pudo completar la operación.');
                }
            }
            closeConfirm();
        }
        ...
        async function init() {
            ...
            try {
                await detectSchema();
                if (!isSchemaComplete()) {
                    console.warn('[GarageOS] Esquema incompleto. Ejecuta supabase/schema.sql en el SQL Editor de Supabase.');
